...
#include<iostream>
using namespace std;
void v1_Data_overlay_ratioplot8bins(string file_name, string file_name_test, string file_name_data, string pathname)                                                                       
{
...
//  const char *data_1[3]={"semiparam ","nosemi","DRN (E)"};
    const char *data_1[3]={"Ratio_NSM","logRatioFlip_MSI","2agg_5M_MSI_logRatio_t4"};

  const char* tag[3]={"Simulation","Simulation","Data"};

  const char *Energy[8]={"20","50","80","100","120","200","250","300"};
    int Elist2[8]={20,50,80,100,120,200,250,300};
  ...
  for (int i_data=0; i_data<3;i_data++)
    {
      ...
      sprintf(full_path1,"%s/%s_reso_semi_nosemi_ep100.png",path2,data[i_data]);                                            
      sprintf(full_path2,"%s/%s_resp_semi_nosemi_ep100.png",path2,data[i_data]);
      sprintf(full_path3,"%s/%s_reso_semi_nosemi_ep100.pdf",path2,data[i_data]);
      sprintf(full_path4,"%s/%s_resp_semi_nosemi_ep100.pdf",path2,data[i_data]);
    ...
    //TLegendEntry* l12 = legend->AddEntry(h_resolution_shubham,data_1[2],"ep");
	  ...
	  h_resolution_2feat->SetPoint(ibin, Elist2[ibin],ratio_reso2feat);
	  h_resolution_1feat->SetPoint(ibin, Elist2[ibin],ratio_reso1feat);
	  h_response_2feat->SetPoint(ibin, Elist2[ibin],ratio_resp2feat);
	  h_response_1feat->SetPoint(ibin, Elist2[ibin],ratio_resp1feat);
...
mg->Draw("ALP");legend->Draw("sames");gPad->Modified();
  ...
  canvas_n1->cd();
   ...
   mg_ratio->GetYaxis()->SetRangeUser(0.9,1.4);
  ...
  mg_ratio->Draw("ALP");
TLine* l_2=new TLine(0,1.0,300 ,1.0);
  l_2->Draw("sames");
...
//   mg1_ratio->Draw("ALP");
  ...
  TLine* l_1=new TLine(0,1.0,300,1.0);
  l_1->Draw("sames");
  //  legend->Draw("sames");
  gPad->Modified();
  gPad->Update();
