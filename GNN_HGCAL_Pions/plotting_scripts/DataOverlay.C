...
void DataOverlay(string file_name)//_valid, string file_name_test, string file_name_data)
{
  ...
  sprintf(path2,"Results/v1",file_name.c_str());
  ...
  sprintf(full_path1,"%s/overlay_resolution_.png",path2);
  sprintf(full_path2,"%s/overlay_response_.png",path2);

  sprintf(full_path11,"%s/overlay_resolution_.pdf",path2);
  sprintf(full_path22,"%s/overlay_response_.pdf",path2);
 ...
 TGraphErrors* h_resolution_data = (TGraphErrors*)inputfile->Get(hist_name5);
  ...
  h_resolution_valid->SetMarkerSize(1.4);
  ...
  h_resolution_test->SetMarkerSize(1.4);
  ...
  h_resolution_data->SetMarkerSize(1.4);
  h_resolution_data->SetMarkerStyle(kFullSquare);

   h_resolution_data->GetXaxis()->SetTitleSize(0.045);

   h_resolution_data->GetYaxis()->SetTitleSize(0.045);

   h_resolution_data->GetXaxis()->SetTitleOffset(0.9);
   h_resolution_data->GetYaxis()->SetTitleOffset(1.1);
  ...
  h_response_valid->SetMarkerSize(1.4);
  ...
  h_response_test->SetMarkerSize(1.4);
  ...
  h_response_data->SetMarkerSize(1.4);
  h_response_data->SetMarkerStyle(kFullSquare);

   h_response_data->GetXaxis()->SetTitleSize(0.045);

   h_response_data->GetYaxis()->SetTitleSize(0.045);

   h_response_data->GetXaxis()->SetTitleOffset(0.9);
   h_response_data->GetYaxis()->SetTitleOffset(1.1);
  ...
  axis5->SetRangeUser(0.01,0.25);
  canvas_n1->SetGrid();
  auto legend = new TLegend(0.45,0.65,0.75,0.85);
legend->SetLineColor(kWhite);
  legend->SetHeader("","C");
  legend->AddEntry(h_resolution_valid,"Validation","p");
  ...
  gPad->Modified();
  gPad->Update();
  h_resolution_test->SetMinimum(0.01);
  h_resolution_test->SetMaximum(0.25);
  ...
  h_response_test->Draw("ALP");
   auto legend1 = new TLegend(0.45,0.65,0.75,0.85);
legend1->SetLineColor(kWhite);
  legend1->SetHeader("","C");
  legend1->AddEntry(h_resolution_valid,"Validation","p");
  legend1->AddEntry(h_resolution_test,"Training","p");
 legend1->AddEntry(h_resolution_data,"Data","p");
 gStyle->SetLegendTextSize(0.05);
  mg1->Draw("LP");
  TLine* l=new TLine(0,1,330,1);
  l->Draw("sames");
  legend1->Draw("sames");
  gPad->Modified();
  gPad->Update();
